...
use crate::stable_manager::SharedStableManager;
use crate::util::proto_adapter::to_error_response;

pub(crate) type SharedPushService = std::sync::Arc<std::sync::Mutex<PushService>>;
...
pub(crate) struct Context {
	...
	pub(crate) stable_manager: SharedStableManager,
	pub(crate) push_service: SharedPushService,
}
...
impl Service<Request<Incoming>> for NodeService {
	...
	fn call(&self, req: Request<Incoming>) -> Self::Future {
		...
		let context = Context {
			...
			stable_manager: Arc::clone(&self.stable_manager),
			push_service: Arc::clone(&self.push_service),
		};
		...
		match &req.uri().path()[1..] {
			...
			LIST_STABLE_CHANNELS_PATH => Box::pin(handle_request(
				context,
				req,
				auth_params,
				api_key,
			...
			EDIT_STABLE_CHANNEL_PATH => Box::pin(handle_request(
				context,
				req,
				auth_params,
				api_key,
			...
			REGISTER_PUSH_PATH => Box::pin(handle_request(
				context,
				req,
				auth_params,
				api_key,
				handle_register_push_request,
			)),
			AUDIT_LOG_PATH => Box::pin(handle_request(
				context,
				req,
				auth_params,
				api_key,
