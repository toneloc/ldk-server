...
pub fn init_db(data_dir: &Path) {
    ...
    let conn = match Connection::open(&path) {
        Ok(c) => c,
        Err(e) => { error!("[push-tokens] Failed to open DB: {}", e); return; }
    };
...
pub fn load_token_for_node(data_dir: &str, node_id: &str) -> Option<TokenInfo> {
    ...
    let cutoff = std::time::SystemTime::now()
        .duration_since(std::time::UNIX_EPOCH)
        .unwrap_or_default()
        .as_secs() as i64 - (30 * 86400); // 30 days
    ...
    conn.query_row(
        ...
        |row| {
            Ok(TokenInfo {
                token: row.get(0)?,
                platform: row.get(1)?,
                environment: row.get::<_, Option<String>>(2)?.unwrap_or_else(|| "sandbox".to_string()),
            })
        },
    ).ok()
}
