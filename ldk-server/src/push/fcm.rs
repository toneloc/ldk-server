...
impl FcmService {
		...
		let credentials = if cred_path.exists() {
			match std::fs::read_to_string(&cred_path) {
				Ok(contents) => match serde_json::from_str::<FcmCredentials>(&contents) {
					...
					Err(e) => {
						warn!("[fcm] Failed to parse service account: {}", e);
						None
					},
				},
				Err(e) => {
					warn!("[fcm] Failed to read {}: {}", cred_path.display(), e);
					None
				},
			}
		} else {
	...
	pub async fn send(&self, token: &str, direction: &str, node_id: &str) {
		let creds = match &self.credentials {
			Some(c) => c,
			None => {
				warn!("[fcm] No credentials available");
				return;
			},
		};
		...
		let url =
			format!("https://fcm.googleapis.com/v1/projects/{}/messages:send", creds.project_id);
		...
		match reqwest::Client::new().post(&url).bearer_auth(&access_token).json(&body).send().await
		{
			Ok(resp) => {
...
async fn generate_access_token(creds: &FcmCredentials) -> Option<String> {
	...
	let b64_encode = |data: &[u8]| -> String {
		base64::encode_block(data).replace('+', "-").replace('/', "_").replace('=', "")
	};
