use a2::{Client as ApnsClient, ClientConfig as ApnsClientConfig, DefaultNotificationBuilder, NotificationBuilder, NotificationOptions, Priority};
    ...
    pub fn new(data_dir: &Path) -> Self {
        ...
        let sandbox_client = key_data.as_ref().and_then(|data| {
            match ApnsClient::token(
                ...
                Ok(c) => Some(c),
                Err(e) => { warn!("[apns] Failed to create sandbox client: {}", e); None }
            }
        });
        ...
        let production_client = key_data.as_ref().and_then(|data| {
            match ApnsClient::token(
                ...
                Ok(c) => Some(c),
                Err(e) => { warn!("[apns] Failed to create production client: {}", e); None }
            }
        });
